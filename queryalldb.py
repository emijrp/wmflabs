...
def main():
    #This script receives a MySQL query as first parameter
    #and runs it over all Wikimedia projects
    #Modify it to do cool stuff with the data instead of print it on screen
    #Remove or add wiki families as desired
    
    #default query or get query by parameter
    query = "SELECT ss_total_edits FROM site_stats WHERE 1"
    if len(sys.argv) > 1:
        query = sys.argv[1]
    
    #get all wiki databases locations and names
    ...
    for row in result:
        ...
        #run the same query over every database
        try:
